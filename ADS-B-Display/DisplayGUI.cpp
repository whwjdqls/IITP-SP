...
//     while (true)
//     {
//         if (!ReadFile(hStdOutRead, buffer, bufSize - 1, &bytesRead, NULL) || bytesRead == 0)
//             break;
...
// std::string RunPythonScriptAndGetOutput(const std::string& scriptPath, const std::string& args)
// {
//     // 절대경로 Python 사용
//     std::string pythonPath = "C:\\Users\\JungBinCho\\AppData\\Local\\Programs\\Python\\Python39\\python.exe";
//     std::string command = "\"" + pythonPath + "\" \"" + scriptPath + "\" " + args;

//     std::string result;
//     char buffer[128];

//     // _popen으로 명령 실행, 읽기 모드
...
//     // 명령어: python "scriptPath" args
//     std::string command = "\"" + pythonPath + "\" \"" + scriptPath + "\" " + args;

//     std::string result;
//     char buffer[256]; // 버퍼를 조금 키워 안정성 향상
...
//     std::string result;
//     char buffer[512]; // 버퍼 확장: JSON 출력 안정성
...
//--------------------------------------------------------------------------
    ...
    // std::string commandLine = "\"" + pythonPath + "\" \"" + scriptPath + "\" " + args;
        AnsiString commandLine = "python " + scriptPath+" "+args;
		// AnsiString commandLine = "C:\\Users\\JungBinCho\\AppData\\Local\\Programs\\Python\\Python39\\python.exe "+scriptPath+" "+args;
        char* cmdLineCharArray = new char[strlen(commandLine.c_str()) + 1];
		strcpy(cmdLineCharArray, commandLine.c_str());
	#define  LOG_PYTHON 1
	#if LOG_PYTHON
...
//     //    Memo1->Lines->Add("DEBUG: Running Python script...");
//     //});
...
//     // 디버깅: Python 실행 후
//     //TThread::Queue(nullptr, [this, llmResponse]() {
//     //    Memo1->Lines->Add("DEBUG: Python finished, response received.");
...
void __fastcall TForm1::LIstenClick(TObject *Sender)
{
    ...
    try
    {
        // SpSharedRecoContext1을 명시적으로 연결
    ...
    catch (Exception &e)
    {
        ...
        ShowMessage("Speech recognition initialization error: " + e.Message);
    }
    catch (...)
    {
        Memo1->Lines->Add("An unknown error occurred.");
        ShowMessage("An unknown error occurred during speech recognition initialization.");
    }
}
//---------------------------------------------------------------------------
